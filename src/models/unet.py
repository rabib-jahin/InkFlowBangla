...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        # === NEW: If no text encoder, create an internal Embedding Layer ===
        if text_encoder is None:
            # vocab_size: গ্রাফিম সংখ্যা (যা new.py থেকে আসবে)
            # context_dim: ভেক্টর সাইজ (সাধারণত 320 বা 768)
            self.token_embedding = nn.Embedding(vocab_size, context_dim)
            print(f"✅ Created internal Token Embedding: {vocab_size} -> {context_dim}")
        # ==================================================================
    ...
    def forward(self, x, timesteps=None, context=None, y=None, mix_rate=None, style_extractor=None, **kwargs):
        ...
        if context is not None:
            ...
            if self.text_encoder is None:
                # Case 1: Grapheme Tokenizer (Integer Input)
                # context shape: [Batch, Seq_Len] -> [Batch, Seq_Len, Dim]
                context = self.token_embedding(context)
            else:
                # Case 2: Canine/BERT (Legacy Dictionary Input)
                context = self.text_encoder(**context).last_hidden_state
                
                # Optional projection for legacy models
