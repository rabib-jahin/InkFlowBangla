...
def train(diffusion, model, ema, ema_model, vae, optimizer, mse_loss, loader, test_loader, 
    ...
    import time
    import gc
    matra_criterion = MatraConsistencyLoss(device=args.device)
    ...
    for epoch in range(start_epoch,args.epochs):
        ...
        matra_weight = 0.1 * min(1.0, epoch / 20)
        
        for i, data in enumerate(pbar):
            images = data[0].to(args.device)
            pixel_images = images.clone()
            transcr = data[1]
            s_id = data[2].to(args.device)
            style_data = data[3]  # Either style_images or style_features
            ...
            if args.model_name == 'wordstylist':
                ...
                for text in transcr:
                    encoded = tokenizer.encode(text) # টেক্সট -> লিস্ট অফ ইন্টিজার
                    batch_indices.append(torch.tensor(encoded))
            
            # টেনসর তৈরি [Batch_Size, 95]
            ...
            if use_cached_features:
                # style_data is already features [B, N, feat_dim]
                style_features = style_data.to(args.device)
                
                # Reshape to match expected format: [B*N, feat_dim]
                B, N, feat_dim = style_features.shape
                style_features = style_features.reshape(B * N, feat_dim)
                
                # The model expects [B*5, feat_dim] just like when extracting from images
            else:
                # Original: extract features from images
            ...
            predicted_noise = model(x_t, timesteps=t, context=text_features, y=labels, 
                                   style_extractor=style_features)
            loss = mse_loss(noise, predicted_noise)
            matra_loss_val = torch.tensor(0.0).to(args.device)
            alpha_bar = diffusion.alpha_hat[timesteps][:, None, None, None]
            pred_x0 = (noisy_images - torch.sqrt(1 - alpha_bar) * predicted_noise) / torch.sqrt(alpha_bar)
          
            if args.latent== True: # ল্যাটেন্ট না হলে আমরা মাত্রা চেক করব
                # print("here------------")
                # x0 রিকভারি সূত্র
                alpha_bar = diffusion.alpha_hat[t][:, None, None, None]
                pred_x0 = (x_t - torch.sqrt(1 - alpha_bar) * predicted_noise) / torch.sqrt(alpha_bar)
                # matra_loss_val = matra_criterion(pred_x0, images)
                # print(matra_loss_val)
                slice_size = 4 
                
                # ১. স্লাইস করা (প্রথম ৪টি)
                pred_slice = pred_x0[:slice_size]
                target_slice = pixel_images[:slice_size] # Real images (Pixels)

                # ২. ল্যাটেন্ট স্কেলিং ঠিক করা (VAE এর নিয়ম অনুযায়ী)
                pred_slice = pred_slice / 0.18215

                # ৩. ডিকোড করা (Latent -> Pixel Image)
                # DataParallel থাকলে .module লাগে, না থাকলে সরাসরি
                if hasattr(vae, 'module'):
                    decoded_img = vae.module.decode(pred_slice).sample
                else:
                    decoded_img = vae.decode(pred_slice).sample
                
                # ৪. এখন আমরা পিক্সেল ইমেজ পেয়ে গেছি, তাই Matra Loss কাজ করবে
                matra_loss_val = matra_criterion(decoded_img, target_slice)
            
            else:
                # যদি কখনো পিক্সেল মোডে চালান
                matra_loss_val = matra_criterion(pred_x0, images)

            # 3. Total Loss (Backpropagate this!)
            # total_loss = + (matra_weight * matra_loss_val)
                
            # টোটাল লস
            total_loss = loss + (matra_weight * matra_loss_val)
            ...
            # Global Step ক্যালকুলেশন (গ্রাফের X-axis ঠিক রাখার জন্য)
                global_step = epoch * len(loader) + i
                
                # ১. আলাদা আলাদা মেট্রিক লগ করা
                experiment.log_metric("matra_loss", matra_loss_val.item(), step=global_step)
                experiment.log_metric("mse_loss", loss.item(), step=global_step)
                experiment.log_metric("total_loss", total_loss.item(), step=global_step)

            # ২. Matra Weight লগ করা (যাতে বুঝতে পারেন ওয়েট বাড়ছে কি না)
                experiment.log_metric("matra_weight_schedule", matra_weight, step=global_step)
            # experiment.log_metrics({
            #     "mse_loss": loss.item(),
            #     "matra_loss": matra_loss_val.item(),
        ...
        print(f"\n⏱️  Epoch time: {epoch_time:.2f}s ({epoch_time/60:.2f} min)")
        print(f"📊 Average Loss: {loss_meter.avg:.4f}")
        if lr_scheduler is not None:
            lr_scheduler.step()
            print(f"📉 Current LR: {lr_scheduler.get_last_lr()[0]:.8f}") # চেক করার জন্য প্রিন্ট
        # === IMAGE LOGGING EVERY 2 EPOCHS ===
        ...
        if epoch % 2 == 0:
            ...
            # Handle both cached and non-cached test data
            if use_cached_features:
                batch_styles = test_data[3]  # Cached features
            else:
                batch_styles = test_data[3]  # Style images
            ...
            for idx in range(num_writers_to_log):
                ...
                # Prepare style input for generation
                if use_cached_features:
                    # Use cached features
                    style_ref = batch_styles[selected_indices[0]].unsqueeze(0)  # [1, N, feat_dim]
                    style_input_feat = style_ref.repeat(n_gen_words, 1, 1).to(args.device)  # [n_gen_words, N, feat_dim]
                    
                    # Reshape to [n_gen_words * N, feat_dim] to match training format
                ...
                if args.model_name == 'wordstylist':
                    ...
                    # === FIX FOR GRAPHEME TOKENIZER ===
                    batch_gen_indices = []
                    for word in gen_words:
                        # ১. শব্দটিকে এনকোড করুন (list of integers)
                        encoded = tokenizer.encode(word) 
                        batch_gen_indices.append(torch.tensor(encoded))
                    
                    # ২. টেনসর স্ট্যাক করুন এবং ডিভাইসে পাঠান
                ...
                # Extract style features if not using cache
                if not use_cached_features and style_extractor:
...
def main():
    ...
    if args.load_check == True:
        print('\n📥 Loading checkpoints...')
        unet.load_state_dict(torch.load(f'{args.save_path}/models/42_ckpt.pt'))
        optimizer.load_state_dict(torch.load(f'{args.save_path}/models/42_optim.pt'))
        ema_model.load_state_dict(torch.load(f'{args.save_path}/models/42_ema_ckpt.pt'))
        print('✅ Loaded models and optimizer')
        start_epoch=0
        for group in optimizer.param_groups:
            group.setdefault('initial_lr', 0.0001)
            
            start_epoch = 50# Set the epoch to resume from
